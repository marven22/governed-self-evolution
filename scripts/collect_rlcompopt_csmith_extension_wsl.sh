...
exec "$venv/bin/python" "$root/scripts/collect_rlcompopt_blame_gated_transactions.py" \
  ...
  --split "$root/configs/rlcompopt_csmith_adaptation_extension_v1.json" \
  --cohort development \
  --certificate-profile csmith \
  --seed-ledger "$run/csmith_adaptation_development_ledger_v1.json" \
  --output "$run/csmith_adaptation_development_ledger_v2.json" \
  --candidate-budget 12 \
  --parent-ranks 0,1 \
  --donor-limit 10
