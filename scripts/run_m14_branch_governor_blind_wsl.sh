#!/usr/bin/env bash
# Frozen governor evaluation on V5 blind parents 135--138 only.
set -euo pipefail
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"; BANK="/home/vmargapu/experiments/m14_coge_stratified_bank_v1"; ROOT="/home/vmargapu/experiments/m14_branch_governor_blind_v1"; DEV="/home/vmargapu/experiments/m14_branch_certify_replay_v1"; PY="/home/vmargapu/tdmpc2-metaworld-official/bin/python"; MODEL="$ROOT/governor.json"
...
for P in 135 136 137 138; do for S in bc r1 r3; do
 ...
 [ -f "$PLAN" ] || "$PY" "$REPO_ROOT/scripts/plan_m14_branch_governor_v1.py" --checkpoint "$C" --model "$MODEL" --candidates "$REPO_ROOT/configs/m14_coge_micro_candidates_v31.json" --controller-id "p$P-$S" --controller-seed "$P" --seed $((P*1000+${#S})) --output "$PLAN"
