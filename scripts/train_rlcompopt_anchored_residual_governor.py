...
def main():
 p=argparse.ArgumentParser();p.add_argument('--ledger',type=Path,required=True);p.add_argument('--base',type=Path,required=True);p.add_argument('--model',type=Path,required=True);p.add_argument('--report',type=Path,required=True);p.add_argument('--bootstrap',type=int,default=32);a=p.parse_args()
 ...
 x=np.vstack(xs); y=np.asarray(ys); model=make_pipeline(StandardScaler(),BayesianRidge());model.fit(x,y); rng=np.random.default_rng(20260922); ensemble=[]
 for _ in range(a.bootstrap):
  idx=rng.integers(0,len(y),len(y)); m=make_pipeline(StandardScaler(),BayesianRidge());m.fit(x[idx],y[idx]);ensemble.append(m)
 payload={'protocol':'rlcompopt-anchored-residual-v1','base_model':str(a.base),'advantage_model':model,'bootstrap_ensemble':ensemble,'margin':.01,'contract':'override V2 only when predicted advantage minus beta times bootstrap standard deviation exceeds margin; otherwise retain V2'}
 a.model.parent.mkdir(parents=True,exist_ok=True);a.report.parent.mkdir(parents=True,exist_ok=True)
 pickle.dump(payload,a.model.open('wb')); report={'protocol':payload['protocol'],'contexts':len(groups),'rows':len(y),'positive_override_examples':int((y>.01).sum()),'mean_residual_advantage':float(y.mean()),'bootstrap_models':len(ensemble),'margin':.01,'limitation':'Training-only artifact; requires blocked calibration before any promotion.'};a.report.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report))
