...
def main() -> None:
 p=argparse.ArgumentParser(); p.add_argument('--checkpoint',type=Path,required=True); p.add_argument('--model',type=Path,required=True); p.add_argument('--candidates',type=Path,required=True); p.add_argument('--labels',nargs='+',default=['MICRO_25_25R','MICRO_25_50R','MICRO_25_75R']); p.add_argument('--controller-id',required=True); p.add_argument('--controller-seed',type=int,required=True); p.add_argument('--seed',type=int,required=True); p.add_argument('--output',type=Path,required=True); p.add_argument('--episodes',type=int,default=50); a=p.parse_args()
 ...
 for c in load_candidates(a.candidates):
  if c['label'] not in a.labels: continue
