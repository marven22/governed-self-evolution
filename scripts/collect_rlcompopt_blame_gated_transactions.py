...
def main() -> None:
    ...
    try:
        for benchmark in benchmarks:
            ...
            parent_index = ranked[0]
            parent_actions = coreset[parent_index]
            donors = [coreset[index] for index in ranked[: min(5, len(ranked))]]
            ...
            # A parent with a repeatable, certified non-negative reward is a
            # trusted baseline for blame attribution.  A weak parent can still
            # generate data, but child disagreement is not charged as blame.
            ...
            for edit in candidates:
                ...
                transactions.append({
                    ...
                    "controller": {
                        ...
                        "ranked_donor_indices": ranked[: min(5, len(ranked))],
    ...
    print(json.dumps({"transactions": len(transactions), "labels": dict(sorted(labels.items())), "output": str(args.output)}, sort_keys=True))
