...
def main():
 ...
 for step in range(a.max_commits):
  ...
  if not accepted:
   history.append({'step':step,'committed':False,'branches':[{'label':x[0],'screen_delta':x[2].utility_delta,'promising':x[4],'confirmed':x[3].committed} for x in candidates]});break
